...
每天北京时间 09:00 自动生成一份精简中文 AI 报告，重点覆盖：
...
Markdown 和邮件使用同一份一屏内容：顶部只有一条 14～24 字的当日焦点，下面最多显示 10 条完整、可直接点击的精炼中文标题，不按固定字数截断；长标题在手机上自动换行。不显示日期、分类、来源、时间、单条介绍或“为什么值得看”。邮件以 14px 字号、深蓝灰无下划线链接和紧凑间距的 HTML 列表为主要正文，同时保留纯文本兼容层，不发送任何附件。

工作流每天也会自动运行。GitHub cron 使用 UTC，当前 `0 1 * * *` 对应北京时间 09:00，平台繁忙时可能延迟几分钟。

## 大模型怎么调用

不配置模型也能直接运行：系统会用确定性规则挑选并生成全中文报告。配置模型后，每份报告最多调用一次，用于从候选中选出最多 10 条精华并精炼标题。
